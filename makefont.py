...
vari_dir = './kanji-database/variants/'

work_dir = "HanaMinA.work"
...
def add_posSub(glyph, feat, val):
    ...
    if glyph.glyphname == "":
        print ("add_posSub", "glyph name is empty");
    else:
...
def make_vkrn (font, reg1, reg2, vkrn):
    ...
    for file1 in os.listdir(work_dir):
        ...
        if (m1):
            for file2 in os.listdir(work_dir):
...
def make_variants(font,regex1,regex2,regex3):
    global vari_dir
    #tmp = u"^("+regex1+u"),"+regex3+u",("+regex2+u")";
